...
paru -S --needed --noconfirm \
    ...
    firefox kitty gnome-text-editor gnome-screenshot oh-my-posh \
    spotify niri xwayland-satellite gnome-keyring corectl \
    networkmanager vesktop 

# Step 3: Example file copy (customize paths!)
SRC_FILE="$HOME/Downloads/example.con"
DEST_DIR="$HOME/.config/example"
DEST_FILE="$DEST_DIR/example.conf"

COPY_NIRI="$HOME/noctalia-shell-expanded/config.kdl"
COPY_FUZZEL=
COPY_DESKTOP=
COPY_IMAGE=
COPY_
